...
class App(QMainWindow):
    ...
    def Rpad_ui(self):
        ...
        self.setWindowTitle('{} - Notepad'.format(self.fname))
    ...
    def edit_menu(self):
        ...
        find_action = QAction(QtGui.QIcon('find.png'), '&Find', self)
        find_action.setStatusTip('Find a string')
        find_action.setShortcut('Ctrl+F')
        find_action.triggered.connect(self.find_action)

        select_all_action = QtWidgets.QAction(QtGui.QIcon('select_all.png'), '&Select All', self)
            ...
            if file_path:
                if (self.file_path[0].split('/')[-1].lower()== self.fname.lower()):
                    ...
                    with file_open:
                        ...
                        self.need_saving(False)
                    return
                    #self.statusBar().showMessage('Add Extension to the file')
                    #fname = QFileDialog.getSaveFileName(self,'Save File','./',filter='All Files(*.*);; Text Files(*.txt)')
